 ...
 * @pcm_frames_per_period: the number of PCM frames transferred in an interval
 *			   of interrupts generated by 1394 OHCI.
 *
		       ...
		       unsigned int pcm_frames_per_period)
{
	...
	/*
	 * This calculation is rough for blocking transmission because of empty
	 * packets and packets with NODATA. For non-blocking transmittion, this
	 * expects devices to process packets as early as possible after data
	 * is sampled.
	 */
	if (pcm_frames_per_period > 0) {
	        s->packets_per_period =
			CYCLES_PER_SECOND * pcm_frames_per_period /
